...
import { setIdentity, generateCode } from "@/lib/game-storage";
  ...
  head: () => ({
    meta: [
      { title: "Recruiting BIAS – Das Spiel" },
      {
        name: "description",
        content:
          "Ein Kartenspiel über unbewusste Vorurteile und Diskriminierung im Recruiting – jetzt digital, live & im Team spielbar.",
      },
      { property: "og:title", content: "Recruiting BIAS – Das Spiel" },
      {
        property: "og:description",
        content:
          "Wissens-, Wahr-oder-Falsch- und Aktionskarten. Spielt gemeinsam und reflektiert eure Recruiting-Praxis.",
      },
    ],
  }),
  component: Home,
    ...
    try {
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        ...
        const { data, error } = await supabase
          .from("game_sessions")
          .insert({ code, host_name: hostName.trim(), total_rounds: totalRounds })
        ...
        setIdentity(code, { kind: "host", token: data.host_token, name: hostName.trim() });

        // Also create a host player row so host shows in the player list with a score
        const { data: player } = await supabase
          .from("session_players")
          .insert({ session_id: data.id, name: hostName.trim(), is_host: true })
